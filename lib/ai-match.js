...
async function findCrossReferenceMatch(oe, rowsBySheet, resultKind, deadline) {
...
export async function matchOemAgainstSheets({ oe, sheetNames, resultKind = 'availability', deadline }) {
  if (!oe) {
  ...
  const level2 = await findCrossReferenceMatch(oe, rowsBySheet, resultKind, deadline);
  if (level2) {
    return buildResult({
