...
${tableDump}`;
  ...
  // Бюджет 15с: это второй, дополнительный вызов Gemini в пайплайне (после VIN-lookup на 30с) —
  // суммарно должны уложиться в maxDuration Vercel (60с) с запасом на чтение таблиц, см. lib/gemini.js.
  const parsed = await callGeminiSearchJson(prompt, { timeoutMs: 15000 });
